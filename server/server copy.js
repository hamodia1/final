  ...
  try {
    ...
    try {
      ...
      `);
            // Create the results table within the new schema
            await client.query(`
            CREATE TABLE ${schemaName}.assigments (
              skill VARCHAR(255) NOT NULL,
              day VARCHAR(255) NOT NULL,
              time_begin TIME NOT NULL,
              time_end TIME NOT NULL,
...
app.post('/optimize-schedule', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Fetch data from shift_require and shifts_types tables, excluding the `id` column
    ...
    // Convert the data to CSV format, excluding the `id` column
    ...
    formData.append('req_shifts', shiftRequirementsCsv, 'req_shifts.csv'); // Append shift requirements CSV
    formData.append('shifts', shiftsCsv, 'shifts.csv'); // Append shifts CSV
    ...
    // Check if the Flask server returned a solution
    if (flaskResponse.status === 200 && flaskResponse.data) {
      // Access the 'solution' field which contains the CSV string
      const solutionCsv = flaskResponse.data.solution; 
    
      console.log("Received solution CSV from Flask server:");
     
      
      // Insert the results into the 'results' table in the database
      await pool.query(`DELETE FROM ${schemaName}.results`); // Clear previous results
      ...
      // Convert the CSV to a JSON array for easy processing
      const solutionRows = csvToJson(solutionCsv); // Make sure solutionCsv is passed correctly as a string
      ...
      res.json({ message: 'Schedule optimized and results saved in the database.' });
    } else {
